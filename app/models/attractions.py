...
class Image(BaseModel):
    ...
    keywords: List[str] = Field(default_factory=list, description="Image keywords")

class ServiceTime(BaseModel):
    """Service time model."""
    name: str = Field(..., description="Service time name")
    description: Optional[str] = Field(None, description="Service time description")
    days: List[str] = Field(default_factory=list, description="Days of the week")
    startTime: str = Field(..., description="Start time")
    endTime: str = Field(..., description="End time")
    effectiveDate: Optional[str] = Field(None, description="Date when this service time becomes effective")
    expireDate: Optional[str] = Field(None, description="Expiration date of this service time")
...
class Attraction(BaseModel):
    ...
    alternateNames: List[str] = Field(default_factory=list, description="Alternative names")
    description: str = Field(..., description="Detailed description")
    position: Position = Field(..., description="Geographic position")
    classes: List[int] = Field(default_factory=list, description="Attraction classification codes")
    postalAddress: Optional[PostalAddress] = Field(None, description="Postal address")
    telephones: List[Telephone] = Field(default_factory=list, description="Contact telephone numbers")
    images: List[Image] = Field(default_factory=list, description="Associated images")
    serviceTimes: List[ServiceTime] = Field(default_factory=list, description="Service/opening times")
    trafficInfo: Optional[str] = Field(None, description="Transportation information")
    parkingInfo: Optional[str] = Field(None, description="Parking information")
    isPublicAccess: bool = Field(True, description="Public accessibility indicator")
    isAccessibleForFree: bool = Field(False, description="Free admission indicator")
    fees: List[Fee] = Field(default_factory=list, description="Admission fees")
    website: Optional[str] = Field(None, description="Official website URL")
    socialMediaURLs: List[SocialMedia] = Field(default_factory=list, description="Social media URLs")
