...
async def check_data_sync():
    ...
    required_files = ["AttractionList.json",
                      "AttractionServiceTimeList.json", "AttractionFeeList.json"]
    ...
    if not data_dir.exists() or not data_dir.is_dir():
        ...
        missing_files = [
            f for f in required_files if not (data_dir / f).exists()]

        if missing_files:
            logger.info(
                f"Missing required data files: {', '.join(missing_files)}. Will trigger data sync.")
            needs_sync = True
        else:
            # Check the most recent file's modification date
            data_files = [data_dir /
                          f for f in required_files if (data_dir / f).exists()]
            if data_files:
                most_recent = max(data_files, key=lambda p: p.stat().st_mtime)
                last_mod_date = datetime.datetime.fromtimestamp(
                    most_recent.stat().st_mtime).date()

                if last_mod_date < today:
                    logger.info(
                        f"Data files last updated on {last_mod_date}. Will trigger data sync.")
                    needs_sync = True
                else:
                    logger.info(
                        f"Data files are up to date (last updated: {last_mod_date}).")
    ...
    if not needs_sync:
        try:
            ...
            if attraction_count == 0:
                logger.info(
                    "No attractions found in database. Will trigger data sync.")
                needs_sync = True
            else:
                logger.info(
                    f"Database contains {attraction_count} attractions.")
